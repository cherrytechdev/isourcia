...
const schema = Yup.object().shape({
  ...
  otherMarketMainObj: Yup.string().when("marketMainObj", {
    ...
    then: (schema) =>
      schema.required("Veuillez préciser les autres objectifs principaux"),
    otherwise: (schema) => schema.notRequired(),
  }),

  marketExpSearch: Yup.array().min(
    1,
    "Veuillez sélectionner au moins un objectif d'étude de marché"
  ),
  ...
  otherMarketExpSearch: Yup.string().when("marketExpSearch", {
    ...
    then: (schema) =>
      schema.required(
        "Veuillez préciser les autres objectifs d'étude de marché"
      ),
    otherwise: (schema) => schema.notRequired(),
  }),

  marketOptionComp: Yup.array().min(
    1,
    "Veuillez sélectionner au moins une option concurrentielle"
  ),

  otherMarketOptionComp: Yup.string().when("marketOptionComp", {
    is: (val: string[]) => val?.includes("autre"),
    then: (schema) =>
      schema.required("Veuillez préciser les autres options concurrentielles"),
    otherwise: (schema) => schema.notRequired(),
  }),

  marketTechTools: Yup.array().min(
    1,
    "Veuillez sélectionner au moins un outil technologique"
  ),

  otherMarketTechTools: Yup.string().when("marketTechTools", {
    is: (val: string[]) => val?.includes("autre"),
    then: (schema) =>
      schema.required("Veuillez préciser les autres outils technologiques"),
    otherwise: (schema) => schema.notRequired(),
  }),
});
            ...
            <div className="flex flex-col gap-4">
              ...
              <div className="flex flex-col gap-2 pl-4">
                <div className="flex flex-col gap-2">
                  ...
                  <Field type="date" name="marketDateWish" />
                  <ErrorMessage
                    name="marketDateWish"
                    component="div"
                    className="error-message"
